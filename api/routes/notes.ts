...
router.get("/trash", auth, async (req: any, res: Response) => {
  try {
...
router.put("/trash/recover/:id", auth, async (req: any, res: Response) => {
  if (!mongoose.Types.ObjectId.isValid(req.params.id))
    return res.status(400).json({ error: "Invalid note id" });
...
router.delete("/trash/:id", auth, async (req: any, res: Response) => {
  if (!mongoose.Types.ObjectId.isValid(req.params.id))
    return res.status(400).json({ error: "Invalid note id" });
